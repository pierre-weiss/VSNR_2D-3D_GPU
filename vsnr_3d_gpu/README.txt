...
This repository contains the sources for VSNR GPU 3D.
...
NOTE: THE .dll THAT YOU CAN FIND ON PIERRE WEISS WEBSITE SHOULD WORK ON WINDOWS (YOU SHOULDN'T HAVE TO COMPILE BY YOURSELF ON WINDOWS)
    ...
    LINUX: 
    cd src
    nvcc -o libvsnr3d.so -lcufft -lcublas --compiler-options "-fPIC" --shared vsnr3d.cu
    ...
    /usr/local/cuda/bin/nvcc -v -o libvsnr3d.so -lcufft -L /usr/local/cuda-6.5/lib64/ -lcublas  --compiler-options "-fPIC" --shared vsnr3d.cu

    WINDOWS:
    cd src
    nvcc -o libvsnr3d.dll -L cufftw.lib cufft.lib cublas.lib --shared vsnr3d.cu
...
*** STEP 3/ Plugin  install *** 
  ...
  - copy/move the VSNR_GPU_3D.jar and the libvsnr3d.so (or libvsnr3d.dll) into this folder
  - launch ImageJ (the plugin will be find in "plugin" -> "process" -> "VSNR GPU 3D")
  - enjoy :)
